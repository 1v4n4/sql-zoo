...
SELECT 
   ...
   deaths, RANK() OVER (ORDER BY deaths DESC) rcd
  FROM covid
...
SELECT 
   world.name,
   ROUND(100000*confirmed/population,0) rate, RANK() OVER (ORDER BY confirmed/population) rc
